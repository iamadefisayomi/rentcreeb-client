...
export default function ChatList({
  ...
  useEffect(() => {
    ...
    return () => unsubscribe();
  }, [currentChatId, setOnlineUsers, onUserStatus]);
  ...
  useEffect(() => {
    const unsubscribe = onMessage((message) => {
      setChats((prevChats: any[]) => {
        ...
        // Remove old chat and insert at top if not already there
  ...
  return (
    <div className="w-full flex flex-col gap-1 overflow-y-auto py-4">
      {chats.map(chat => {
        ...
        return (
          <Link href={`${Routes.dashboard.engagement.message}/${chatId}`} key={chatId}>
            <div
              className={cn(
                "flex gap-1 items-center justify-between p-2 hover:bg-slate-100 rounded-lg cursor-pointer w-full overflow-hidden",
                (currentChatId === chatId || currentChatId === property?._id) && "bg-slate-100"
              )}
            >
              <div className="flex-grow flex items-center gap-2">
                <div className="relative rounded-full">
                  <Avatar className="border-2 w-[50px] border-white h-[50px]">
                ...
                <div className="flex flex-col">
                  <h4 className="text-[11px] font-medium truncate capitalize whitespace-nowrap overflow-hidden text-ellipsis">
                    {otherUser?.name}
                  </h4>
                  <p className="text-[10px] text-gray-600 font-semibold truncate whitespace-nowrap overflow-hidden text-ellipsis">
                    {property?.title || ''}
                  </p>
                  <p className="text-[10px] text-muted-foreground truncate whitespace-nowrap overflow-hidden text-ellipsis">
                    {
